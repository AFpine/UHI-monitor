...
function loadMapData(jsonString, HEAT_ISLAND_TEMP1, HEAT_ISLAND_TEMP2, HEAT_ISLAND_AVG, HUMIDITY, FEEL_LIKE_TEMP, SLAT, SLNG, STEMP) {
    ...
    sLAT = JSON.parse(SLAT);
    sLNG = JSON.parse(SLNG);
    ...
    update_infowindow(map, sLAT, sLNG, sTEMP);
}
...
function update_infowindow(map, sLAT, sLNG, sTEMP) {
    if(heatmap) {
        heatmap.setMap(null);
        console.log("hi");
    }
    ...
    var minT = sTEMP[0];
    var maxT = sTEMP[0];
    for(var i = 1; i < sLAT.length; ++i) {
        minT = Math.min(minT, sTEMP[i]);
        maxT = Math.max(maxT, sTEMP[i]);
    }
    for(var i = 0; i < sLAT.length; ++i) {
        sTEMP[i] = (sTEMP[i] - minT) / (maxT - minT);
        sTEMP[i] *= 10;
    }
    for(var i = 0; i < sLAT.length; ++i) {
        data.push(new naver.maps.visualization.WeightedLocation(sLAT[i], sLNG[i], sTEMP[i]));
    }
